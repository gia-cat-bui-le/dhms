...
for idx, ax in enumerate(axes.flat):
    ...
    wedges, texts, autotexts = ax.pie(
        ...
        autopct='%1.1f%%', 
        colors=sns.color_palette("tab10", len(category_counts)), # Gradient color palette
        startangle=140, 
        explode=[0.1 if i == 0 else 0 for i in range(len(category_counts))],  # Explode the largest slice
        shadow=False,  # Add shadow for better visual effect
        wedgeprops={'edgecolor': 'black'}  # Enhance borders for clarity
    )
    
    ax.set_title(titles[idx], size=18, weight='bold')
