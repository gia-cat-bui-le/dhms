...
def parse_and_load_from_model(parser):
    ...
    print(f"model dir name: {os.path.dirname(model_path)}\nargs path: {args_path}")
...
def get_model_path_from_args():
    try:
        ...
        print(f"model path: {dummy_args.model_path}")
...
def add_base_options(parser):
    ...
    group.add_argument(
        "--batch_size", default=128, type=int, help="Batch size during training."
    )
    group.add_argument(
        "--tiny", default=False, type=bool, help="If True, use only part of data"
    )
    group.add_argument(
        "--refine",
        action="store_true",
    ...
    group.add_argument(
        "--inpainting_frames", default=30, type=int, help="inpainting_frames"
    )
    group.add_argument(
        "--refine_scale", default=1.0, type=float, help="inpainting_frames"
    )
...
def add_model_options(parser):
    ...
    group.add_argument(
        "--num_heads", default=4, type=int, help="Number of heads per layer."
    )
    group.add_argument(
