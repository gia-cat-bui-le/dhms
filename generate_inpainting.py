...
if __name__ == "__main__":
            ...
            for _, batch in tqdm(enumerate(dataloader)):
                for i in range(len(batch["filename"])):
                    ...
                    for repeat_time in range(repeat_times):
                        ...
                        motion_final = []
                        motion_final_inpainting = []
                        motion_final.append(sample[0].squeeze().unsqueeze(dim=0).permute(0, 2, 1)[:, :45, :])
                        motion_final_inpainting.append(sample[0].squeeze().unsqueeze(dim=0).permute(0, 2, 1))
                        ...
                        for idx in range(bs - 1):
                            ...
                            motion_final_inpainting.append(sample_2.squeeze().unsqueeze(dim=0).permute(0, 2, 1))
                            motion_final_inpainting.append(sample_1.squeeze().unsqueeze(dim=0).permute(0, 2, 1))
                        ...
                        motion_result = torch.cat(motion_final_inpainting, dim=0)
                        ...
                        if b > 1:
                            ...
                            filename = batch_filename
                            outname = f'{args.output_dir}/inference_inpainting/{"".join(os.path.splitext(os.path.basename(filename))[0])}_{repeat_time}.pkl'
                            out_path = os.path.join("./", outname)
    ...
    unnomarlize(f'{args.output_dir}/inference_inpainting')